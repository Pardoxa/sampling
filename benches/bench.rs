...
pub fn benchmark(c: &mut Criterion) {
    ...
    wl.init_greedy_heuristic(|s| Some(s.head_count()), None)
        .unwrap();
    c.bench_function("bench", |b| {
        b.iter(|| wl.wang_landau_step(|s| Some(s.head_count())))
    });
}
...
pub fn bench_wl_step(c: &mut Criterion) {
    ...
    let ensembles: Vec<_> = (0..interval_count.get())
        .map(|_| CoinFlipSequence::new(n, Pcg64::from_rng(&mut rng)))
        .collect();

    // Now the Wang Landau simulation. First create the struct
    // (here as Vector, since we want to use 3 overlapping intervals)
    let mut wl_list: Vec<_> = ensembles
        .into_iter()
        .zip(hist_list)
...
pub fn bench_wl_step_acc(c: &mut Criterion) {
    ...
    c.bench_function("wl_step_acc", |b| {
        b.iter(|| {
            for wl in wl_list.iter_mut() {
...
pub fn bench_hists(c: &mut Criterion) {
    ...
    let max_val = 100;
    #[allow(deprecated)]
    let mut hist = HistI16::new_inclusive(0, max_val, (max_val + 1) as usize).unwrap();

    let uniform = Uniform::new_inclusive(0, max_val).unwrap();
    let mut rng = Pcg64::seed_from_u64(23894623987612);

    c.bench_function("HistI16", |b| {
        b.iter(|| {
            for _ in 0..100 {
    ...
    let mut hist = BinningI16::new_inclusive(0, max_val, 1)
        .unwrap()
        .to_generic_hist();

    let mut rng = Pcg64::seed_from_u64(23894623987612);

    c.bench_function("GenericHistBinning", |b| {
        b.iter(|| {
            for _ in 0..100 {
                let num = uniform.sample(&mut rng);
    ...
    let mut hist = FastBinningI16::new_inclusive(0, max_val).to_generic_hist();

    let mut rng = Pcg64::seed_from_u64(23894623987612);

    c.bench_function("GenericHistFastBinning", |b| {
        b.iter(|| {
            for _ in 0..100 {
                let num = uniform.sample(&mut rng);
                let _ = hist.count_val(num);
    ...
    let mut hist = HistI16Fast::new_inclusive(0, max_val).unwrap();

    let mut rng = Pcg64::seed_from_u64(23894623987612);

    c.bench_function("HistI16Fast", |b| {
        b.iter(|| {
            for _ in 0..100 {
                let num = uniform.sample(&mut rng);
                hist.increment_quiet(num);
