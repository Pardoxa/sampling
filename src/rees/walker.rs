...
impl<R, Hist, Energy, S, Res> From<RewlWalker<R, Hist, Energy, S, Res>>
    for ReesWalker<R, Hist, Energy, S, Res>
where
    Hist: Histogram,
{
...
impl<R, Hist, Energy, S, Res> ReesWalker<R, Hist, Energy, S, Res> {
        ...
        Hist: Histogram,
    {
        ...
        refined_log_density.extend(self.log_density.iter().zip(self.hist.hist().iter()).map(
            |(&log_d, &h)| {
                if h == 0 {
                    log_d
                } else {
    ...
    R: Rng,
{
        ...
        F: Fn(&mut Ensemble) -> Option<Energy>,
    {
        let mut e = ensemble_vec[self.id].write().expect(
            "Fatal Error encountered; ERRORCODE 0x1 - this should be \
        ...
        Hist: Histogram,
    {
        ...
        let mut e = ensemble_vec[self.id].write().expect(
            "Fatal Error encountered; ERRORCODE 0x3 - this should be \
                impossible to reach. If you are using the latest version of the \
                'sampling' library, please contact the library author via github by opening an \
                issue! https://github.com/Pardoxa/sampling/issues",
        );
...
pub(crate) fn replica_exchange<R, Hist, Energy, S, Res>(
    ...
    Hist: HistogramVal<Energy> + Histogram,
    R: Rng,
{
...
pub(crate) fn get_merged_refined_walker_prob<R, Hist, Energy, S, Res>(
