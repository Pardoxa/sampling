use {
    crate::{
        glue::derivative::*,
        glue_helper::{ln_to_log10, subtract_max},
...
impl<Extra, Ensemble, R, Hist, Energy, S, Res> GlueAble<Hist>
    for ReplicaExchangeEntropicSampling<Extra, Ensemble, R, Hist, Energy, S, Res>
where
    Hist: Clone + Histogram,
{
    fn push_glue_entry_ignoring(&self, job: &mut GlueJob<Hist>, ignore_idx: &[usize]) {
        job.round_trips.extend(self.rees_roundtrip_iter());
...
pub type Rees<Extra, Ensemble, R, Hist, Energy, S, Res> =
    ReplicaExchangeEntropicSampling<Extra, Ensemble, R, Hist, Energy, S, Res>;
...
impl<Ensemble, R, Hist, Energy, S, Res, Extra> Rees<Extra, Ensemble, R, Hist, Energy, S, Res> {
    ...
    pub fn rewl_roundtrip_iter(&'_ self) -> impl Iterator<Item = usize> + '_ {
        self.rewl_roundtrips.iter().copied()
    }
    ...
    pub fn rees_roundtrip_iter(&'_ self) -> impl Iterator<Item = usize> + '_ {
        self.rees_roundtrip_halfes.iter().map(|half| half / 2)
    }
}

impl<Ensemble, R, Hist, Energy, S, Res> From<Rewl<Ensemble, R, Hist, Energy, S, Res>>
    for Rees<(), Ensemble, R, Hist, Energy, S, Res>
where
    Hist: Histogram,
{
...
impl<Extra, Ensemble, R, Hist, Energy, S, Res> Rees<Extra, Ensemble, R, Hist, Energy, S, Res> {
    ...
    pub fn is_finished(&self) -> bool {
        self.walker.iter().all(|w| w.is_finished())
    }
    ...
    Res: Send + Sync,
{
    ...
    pub fn refine(&mut self) {
        self.walker.par_iter_mut().for_each(|w| w.refine());
    }
    ...
    pub fn sweep<F, P>(&mut self, energy_fn: F, extra_fn: P)
    where
    ...
    pub fn simulate_until_convergence<F, P>(&mut self, energy_fn: F, extra_fn: P)
    where
        Ensemble: Send + Sync,
        R: Send + Sync,
        F: Fn(&mut Ensemble) -> Option<Energy> + Copy + Send + Sync,
    ...
    pub fn simulate_while<F, C, P>(&mut self, energy_fn: F, mut condition: C, extra_fn: P)
    where
        Ensemble: Send + Sync,
        R: Send + Sync,
        F: Fn(&mut Ensemble) -> Option<Energy> + Copy + Send + Sync,
    ...
    #[allow(clippy::type_complexity)]
    #[deprecated]
    fn merged_log_probability_helper(
        &self,
    ...
    {
        // get the log_probabilities - the walkers over the same intervals are merged
        let mut log_prob: Vec<_> = self
            .walker
            .par_chunks(self.chunk_size.get())
            .map(get_merged_refined_walker_prob)
            .collect();

        log_prob.par_iter_mut().for_each(|v| {
            subtract_max(v);
        });

        // get the derivative, for merging later
        let derivatives: Vec<_> = log_prob.par_iter().map(|v| derivative_merged(v)).collect();
    ...
    #[deprecated(
        since = "0.2.0",
        note = "will be removed in future releases. Use new method 'derivative_merged_log_prob_and_aligned' or consider using 'average_merged_log_probability_and_align' instead"
    )]
    #[allow(deprecated)]
...
fn combine_container<'a, Ensemble, R, Hist, Energy, S, Res, Extra>(
    rees: &'a [Rees<Extra, Ensemble, R, Hist, Energy, S, Res>],
    merged_probs: &'a [Vec<f64>],
) -> Vec<(&'a [f64], &'a Hist)>
where
    Hist: HistogramVal<Energy> + HistogramCombine,
    Energy: PartialOrd,
{
    let hists: Vec<_> = rees
        .iter()
        .flat_map(|r| {
            r.walkers()
                .iter()
    ...
    let mut container: Vec<_> = merged_probs
        .iter()
        .zip(hists)
...
fn merged_probs<Ensemble, R, Hist, Energy, S, Res, Extra>(
    rees: &[Rees<Extra, Ensemble, R, Hist, Energy, S, Res>],
) -> Vec<Vec<f64>>
where
    Hist: Histogram,
{
    let merged_probs: Vec<_> = rees
        .iter()
        .flat_map(|rees| {
            rees.walkers()
...
fn get_merged_walker_prob<R, Hist, Energy, S, Res>(
    walker: &[ReesWalker<R, Hist, Energy, S, Res>],
) -> Vec<f64>
where
    Hist: Histogram,
{
