use {
    ...
    rayon::prelude::*,
    std::{marker::PhantomData, num::NonZeroUsize, sync::*},
};
...
/// * Hint: Use shorthand `RewlBuilder`
/// ## Notes
...
pub type RewlBuilder<Ensemble, Hist, S, Res> =
    ReplicaExchangeWangLandauBuilder<Ensemble, Hist, S, Res>;
    ...
    Res: Sync + Send,
{
    ...
    pub fn finished_fraction(&self) -> f64 {
        let done = self.finished.iter().filter(|&&f| f).count();
    ...
    pub fn from_ensemble_vec(
        ...
        let step_size = (0..ensembles.len()).map(|_| step_size).collect();

        let mut sweep_size_vec = Vec::with_capacity(ensembles.len());
        sweep_size_vec.extend((0..ensembles.len()).map(|_| sweep_size));
    ...
    where
        ...
        R: Rng + SeedableRng,
    {
        let len = NonZeroUsize::new(hists.len()).ok_or(RewlBuilderErr::Empty)?;
        ...
        Energy: Send + Sync,
    {
        if container.iter().any(|(_, _, e)| e.is_none()) {
            let (hists, ensembles) = container.into_iter().map(|(h, e, _)| (h, e)).unzip();
            return Err(Self {
                ensembles,
                hists,
                walker_per_interval,
        ...
        for (((mut h, mut e, energy), step_size), sweep_size) in container
            .into_iter()
            ...
            .zip(sweep_size.into_iter())
        {
            ...
            for _ in 0..walker_per_interval.get() - 1 {
                ...
                walker.push(RewlWalker::<R, Hist, Energy, S, Res>::new(
                    counter,
                    rng,
                    h.clone(),
                    sweep_size,
            ...
            let rng = R::from_rng(e.rng());
            walker.push(RewlWalker::new(
                counter, rng, h, sweep_size, step_size, energy,
            ));
            counter += 1;
        ...
        walker::RewlWalker<R, Hist, Energy, S, Res>: Send,
    {
        ...
        ensembles
            ...
            .map(|((((mut e, h), &step_size), finished), sweep_size)| {
                ...
                if !h.is_inside(&energy) {
                    ...
                    'outer2: loop {
                        for _ in 0..sweep_size.get() {
                            ...
                            let new_distance =
                                h.interval_distance_overlap(&current_energy, overlap);
                            if new_distance <= distance {
                                energy = current_energy;
                                distance = new_distance;
                                if distance == 0 {
    ...
    where
        ...
        walker::RewlWalker<R, Hist, Energy, S, Res>: Send,
    {
        match Self::try_mixed_heuristik_choose_rng_build(
            self,
            energy_fn,
            || true,
            overlap,
    ...
    where
        ...
        walker::RewlWalker<R, Hist, Energy, S, Res>: Send,
    {
        Self::try_mixed_heuristik_choose_rng_build(
            self,
            energy_fn,
            condition,
            overlap,
