    ...
    R: Rng,
{
    ...
    fn m_steps_acc<Acc, AccFn>(
        ...
        AccFn: FnMut(&Self, &CoinFlipMove, &mut Acc),
    {
        self.steps += 1;
        steps.clear();
...
mod tests {
    ...
    fn combine() {
        ...
        let hist1 = HistUsizeFast::new_inclusive(0, 2 * n / 3).unwrap();

        let hist_list1 = hist1.overlapping_partition(intervals, 1).unwrap();

        let mut rng = Pcg64Mcg::seed_from_u64(384789);

        let ensemble1 = CoinFlipSequence::new(n, Pcg64::from_rng(&mut rng));
        ...
        let ensemble2 = CoinFlipSequence::new(n, Pcg64::from_rng(&mut rng));

        let hist2 = HistUsizeFast::new_inclusive(n / 3, n).unwrap();
        ...
        let steps: u64 = rewl_slice
            .iter()
            .flat_map(|r| r.walkers().iter().map(|w| w.step_count()))
            .sum();
        println!("Ges steps rewl {}", steps);

        let binomial = Binomial::new(0.5, n as u64).unwrap();

        let prob = rewl::merged_log_prob(&rewl_slice).unwrap();
        let ln_prob_true: Vec<_> = (0..=n).map(|k| binomial.ln_pmf(k as u64)).collect();

        let mut rees_slice: Vec<_> = rewl_slice.into_iter().map(|r| r.into_rees()).collect();

        rees_slice.par_iter_mut().for_each(|rees| {
            rees.simulate_until_convergence(|e| Some(e.head_count()), |_, _, _| {})
        });
