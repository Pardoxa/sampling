use {
    crate::{traits::*, *},
    rand::{seq::*, Rng},
    std::{collections::*, convert::*, io::Write, iter::*, marker::PhantomData, num::*},
};
...
impl<Hist, R, E, S, Res, Energy> GlueAble<Hist>
    for EntropicSamplingAdaptive<Hist, R, E, S, Res, Energy>
where
    ...
    R: Rng,
{
    ...
    fn try_from(mut wl: WangLandauAdaptive<Hist, R, E, S, Res, T>) -> Result<Self, Self::Error> {
        ...
        wl.accepted_step_hist.iter_mut().for_each(|v| *v = 0);
        wl.rejected_step_hist.iter_mut().for_each(|v| *v = 0);
    ...
    T: Clone,
{
    ...
    ///   regarding the usage of the mutable ensemble. From a programming point of view this will not lead to
    ///   any undefined behavior or such regardless of if the user fulfills the requirements
    pub unsafe fn entropic_sampling_unsafe<F, G>(&mut self, mut energy_fn: F, mut print_fn: G)
    where
        F: FnMut(&mut E) -> Option<T>,
        G: FnMut(&mut Self),
    {
    ...
    /// * you have access to your ensemble with `self.ensemble()`
    /// * if you do not need it, you can use `|_|{}` as `print_fn`
    pub fn entropic_sampling<F, G>(&mut self, mut energy_fn: F, mut print_fn: G)
    where
        F: FnMut(&E) -> Option<T>,
        G: FnMut(&Self),
    {
    ...
    /// * you have access to your ensemble with `self.ensemble()`
    /// * if you do not need it, you can use `|_|{}` as `print_fn`
    pub fn entropic_sampling_acc<F, G>(&mut self, mut energy_fn: F, mut print_fn: G)
    where
        F: FnMut(&E, &S, &mut T),
        G: FnMut(&Self),
    {
    ...
    ///   regarding the usage of the mutable ensemble. From a programming point of view this will not lead to
    ///   any undefined behavior or such regardless of if the user fulfills the requirements
    pub unsafe fn entropic_step_unsafe<F>(&mut self, mut energy_fn: F)
    where
        F: FnMut(&mut E) -> Option<T>,
    {
        self.step_count += 1;
    ...
    /// # Important
    /// * `energy_fn`: should be the same as used for Wang Landau, otherwise the results will be wrong!
    pub fn entropic_step<F>(&mut self, mut energy_fn: F)
    where
        F: FnMut(&E) -> Option<T>,
    {
        unsafe { self.entropic_step_unsafe(|e| energy_fn(e)) }
    }
        ...
        F: FnMut(&E, &S, &mut T),
    {
        ...
        self.ensemble
            .m_steps_acc(step_size, &mut self.steps, &mut current_energy, energy_fn);
        self.entropic_step_helper(current_energy);
    }
...
impl<Hist, R, E, S, Res, Energy> EntropicEnergy<Energy>
    for EntropicSamplingAdaptive<Hist, R, E, S, Res, Energy>
where
    Hist: Histogram,
    R: Rng,
...
impl<Hist, R, E, S, Res, Energy> EntropicHist<Hist>
    for EntropicSamplingAdaptive<Hist, R, E, S, Res, Energy>
where
    Hist: Histogram,
    R: Rng,
...
impl<Hist, R, E, S, Res, Energy> EntropicEnsemble<E>
    for EntropicSamplingAdaptive<Hist, R, E, S, Res, Energy>
where
    Hist: Histogram,
    R: Rng,
