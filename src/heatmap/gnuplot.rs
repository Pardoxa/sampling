use std::{borrow::*, convert::From, fmt, io::Write, path::Path};
...
impl GnuplotAxis {
    ...
    pub(crate) fn write_tics<W: Write>(
        &self,
        mut w: W,
        num_bins: usize,
        axis: &str,
    ...
    pub fn new(min: f64, max: f64, tics: usize) -> Self {
        let labels = Labels::FromValues { min, max, tics };
        Self {
            labels,
            rotation: 0.0,
        }
    ...
    pub fn from_slice(labels: &[&str]) -> Self {
        let vec = labels.iter().map(|&s| s.into()).collect();
...
impl GnuplotSettings {
    ...
    pub(crate) fn write_terminal<W: Write>(&self, writer: W) -> std::io::Result<()> {
    ...
    pub(crate) fn write_axis<W: Write>(
        &self,
        mut w: W,
        num_bins_x: usize,
        num_bins_y: usize,
        ...
        W: Write,
    {
        ...
        self.write_axis(&mut writer, x_len, y_len)?;
    ...
    {
        self.write_heatmap_helper1(&mut writer, x_len, y_len)?;
    ...
    where
        ...
        P: AsRef<Path>,
    {
        self.write_heatmap_helper1(&mut writer, matrix_width, matrix_height)?;
...
impl CubeHelixParameter {
    ...
    pub fn hue(&mut self, hue: f32) -> &mut Self {
        if valid(hue) {
            ...
            panic!(
                "Invalid hue value! Hue value has to be 0.0 <= hue <= 1.0, you used {}",
                hue
            )
        }
        self
    ...
    pub fn rotation(&mut self, rotation: f32) -> &mut Self {
        if rotation.is_finite() {
            ...
            panic!(
                "Invalid rotation value! Rotation value has to be finite, you used {}",
                rotation
            )
        }
        self
    ...
    pub fn approximate_color_rgb(&self, gray: f32) -> ColorRGB {
        ...
        let color = color.map(|val| (val * 255.0).clamp(0.0, 255.0).floor() as u8);
    ...
    pub(crate) fn write_palette<W: Write>(&self, mut writer: W) -> std::io::Result<()> {
        match self {
            ...
            Self::CubeHelix(helix) => {
                ...
                writeln!(
                    writer,
                    "hue={}     # hue intensity, valid values 0 <= hue <= 1",
                    helix.hue
                )?;
                writeln!(
                    writer,
                    "r={}       # rotation in color space. Typical values: -1.5 <= r <= 1.5",
                    helix.r
                )?;
                writeln!(
                    writer,
                    "s={}       # starting color, valid values 0 <= s <= 1",
                    helix.s
                )?;
                writeln!(writer, "gamma={}   # gamma < 1 emphasizes low intensity values, gamma > 1 high intensity ones", helix.gamma)?;
                writeln!(
                    writer,
                    "low={}     # lowest value for grayscale. 0 <= low < 1 and low < high",
                    helix.low
                )?;
                writeln!(
                    writer,
                    "high={}    # highest value for grayscale. 0< high <= 1 and low < high",
                    helix.high
                )?;
                let s = if helix.reverse { "1" } else { "0" };
                writeln!(
                    writer,
                    "reverse={}   # set to 1 for reverse cbrange, set to 0 for original cbrange",
                    s
                )?;
                writeln!(
                    writer,
