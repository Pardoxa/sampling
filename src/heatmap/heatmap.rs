...
impl<HistWidth, HistHeight> From<HeatmapUsizeMean<HistWidth, HistHeight>>
    for HeatmapU<HistWidth, HistHeight>
{
    ...
    HistHeight: Clone,
{
    ...
    pub fn transpose(&self) -> HeatmapUsize<HistHeight, HistWidth> {
        ...
        transpose(&self.heatmap, &mut transposed, self.width, self.height);
        HeatmapUsize {
    ...
    HistHeight: Histogram,
{
        ...
        OtherHH: Histogram,
    {
        ...
        self.heatmap
            .iter_mut()
            .zip(other.heatmap.iter())
            .for_each(|(this, other)| {
                *this += other;
            });

        for (i, &count) in other.hist_width.hist().iter().enumerate() {
            self.hist_width.increment_index_by(i, count).unwrap()
        }

        for (i, &count) in other.hist_height.hist().iter().enumerate() {
            self.hist_height.increment_index_by(i, count).unwrap()
        }
    ...
    fn any_hit(&self) -> bool {
        let hist_vec = if self.width <= self.height {
            self.hist_width.hist()
        } else {
            self.hist_height.hist()
        };

        hist_vec.iter().any(|&val| val != 0)
    }
    ...
    pub fn bins_hit(&self) -> usize {
        self.heatmap.iter().filter(|&&val| val > 0).count()
    }
    ...
    pub fn vec_normalized(&self) -> Vec<f64> {
        ...
        if total == 0 {
            ...
            res.extend(self.heatmap.iter().map(|&val| val as f64 / total));
                ...
                for y in 0..self.height {
                    ...
                    unsafe {
                        *res.get_unchecked_mut(index) =
                            *self.heatmap.get_unchecked(index) as f64 / denominator;
                    }
                }
            }
        }
    ...
    pub fn vec_normalized_rows(&self) -> Vec<f64> {
        ...
        for y in 0..self.height {
            ...
            let row_sum = row_slice.iter().sum::<usize>();

            if row_sum > 0 {
                let denominator = row_sum as f64;
                let res_slice = &mut res[start_index..fin];
                for (res_val, &heat_val) in res_slice.iter_mut().zip(row_slice.iter()) {
    ...
    where
        ...
        I: Iterator<Item = A>,
    {
        let hight = self.hist_height.get_bin_index(height_val).map_err(|e| {
            self.error_count += 1;
        ...
        for val in width_val_iter {
            counter += 1;
            let x = self.hist_width.count_val(val).map_err(|e| {
                self.error_count += 1;
        ...
        self.hist_height.increment_index_by(hight, counter).unwrap();
    ...
    where
        ...
        B: Borrow<Y>,
    {
        let x = self.hist_width.get_bin_index(width_val).map_err(|e| {
            self.error_count += 1;
            HeatmapError::XError(e)
        })?;
        let y = self.hist_height.count_val(height_val).map_err(|e| {
            self.error_count += 1;
            HeatmapError::YError(e)
        })?;
        ...
        self.hist_width.increment_index(x).unwrap();
    ...
    where
        W: std::io::Write,
    {
        self.gnuplot(writer, GnuplotSettings::default())
    }
    ...
    /// gnuplot heatmap.gp
    /// ```
    pub fn gnuplot<W, GS>(&self, mut gnuplot_writer: W, settings: GS) -> std::io::Result<()>
    where
        W: Write,
        GS: Borrow<GnuplotSettings>,
    {
        ...
        settings.write_heatmap(&mut gnuplot_writer, |w| self.write_to(w), x_len, y_len)
    }
}
...
mod tests {
    ...
    fn equality_test() {
        ...
        let uniform = Uniform::new_inclusive(0, 16).unwrap();
        for i in 0..10 {
    ...
    fn row_test() {
        ...
        let y_distr = Uniform::new_inclusive(0, 6_u8).unwrap();
    ...
    fn plot_test() {
        ...
        let mut rng = Pcg64::seed_from_u64(27456487);
        let x_distr = Uniform::new_inclusive(0, 10_usize).unwrap();
        let y_distr = Uniform::new_inclusive(0, 10_u8).unwrap();
    ...
    fn transpose_test() {
        ...
        let mut rng = Pcg64::seed_from_u64(27456487);
        let x_distr = Uniform::new_inclusive(0, 10_usize).unwrap();
        let y_distr = Uniform::new_inclusive(0, 5_u8).unwrap();
