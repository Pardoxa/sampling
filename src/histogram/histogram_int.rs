...
impl<T> HistogramInt<T>
where
    T: Sub<T, Output = T> + Add<T, Output = T> + Ord + One + Copy + NumCast,
{
    #[inline]
    /// # Increment hit count
...
where
    ...
    T::Unsigned: Bounded
        ...
        + std::ops::Rem<Output = T::Unsigned>
        + FromPrimitive
        + Zero
        + std::cmp::Eq
        + std::ops::Div<Output = T::Unsigned>
        + Ord
        + std::ops::Mul<Output = T::Unsigned>
        + WrappingSub
        + Copy,
    std::ops::RangeInclusive<T::Unsigned>: Iterator<Item = T::Unsigned>,
{
    ...
    pub fn new(left: T, right: T, bins: usize) -> Result<Self, HistErrors> {
        ...
        let bin_borders: Vec<_> = (T::Unsigned::zero()..=b)
            .map(|val| from_u(left_u + to_u(val) * bin_size))
            .collect();
    ...
    T: Ord + Sub<T, Output = T> + Add<T, Output = T> + One + NumCast + Copy,
{
    fn count_val<V: Borrow<T>>(&mut self, val: V) -> Result<usize, HistErrors> {
        ...
        self.increment_index(id).map(|_| id)
    }
    ...
    fn is_inside<V: Borrow<T>>(&self, val: V) -> bool {
        let val = *val.borrow();
        val >= self.first_border() && val < self.get_right()
    }
    ...
    T: Clone + std::fmt::Debug,
{
    fn overlapping_partition(
        ...
        for c in 0..n.get() {
            let left_index = c.checked_mul(size).ok_or(HistErrors::Overflow)? / denominator;

            let zaehler = c + overlap + 1;
            let right_index =
                1 + zaehler.checked_mul(size).ok_or(HistErrors::Overflow)? / denominator;

            if left_index >= right_index {
                return Err(HistErrors::IntervalWidthZero);
            }

            let borders = self.borders()[left_index..=right_index].to_vec();
...
mod tests {
        ...
        HistogramInt<T>: std::fmt::Debug,
    {
        ...
        for (id, i) in (left..=right).enumerate() {
            ...
            assert_eq!(
                hist.interval_distance_overlap(i, unsafe { NonZeroUsize::new_unchecked(2) }),
                0
            );
    ...
    fn overlapping_partition_test() {
        ...
        let uni_n = Uniform::new_inclusive(1, 16).unwrap();
    ...
    fn overlapping_partition_test2() {
        ...
        let uni = Uniform::new_inclusive(-100, 100).unwrap();
        let uni_n = Uniform::new_inclusive(2, 6).unwrap();
        for overlap in 0..=5 {
            for _ in 0..100 {
                let n: usize = uni_n.sample(&mut rng);
                let (left, right) = loop {
                ...
                let hist_fast = HistI8Fast::new_inclusive(left, right).unwrap();
                let hist_i = HistI8::new_inclusive(left, right, hist_fast.bin_count()).unwrap();
                let overlapping_i = hist_i
                    .overlapping_partition(NonZeroUsize::new(n).unwrap(), overlap)
                    .unwrap();
    ...
    fn overlapping_partition_test3() {
        ...
        let uni = Uniform::new_inclusive(-300, 300).unwrap();
        let uni_n = Uniform::new_inclusive(2, 4).unwrap();
        for binsize in 2..=7 {
            for overlap in 0..=5 {
                for _ in 0..100 {
                    let n: usize = uni_n.sample(&mut rng);
                    ...
                    let hist_i =
                        HistI16::new_inclusive(left, right, hist_fast.bin_count() / binsize)
                            .unwrap();
                    let overlapping_i = hist_i
                        .overlapping_partition(NonZeroUsize::new(n).unwrap(), overlap)
                        .unwrap();
