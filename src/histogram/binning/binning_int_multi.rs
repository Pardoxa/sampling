use paste::paste;
use std::{borrow::Borrow, fmt::Debug, ops::RangeInclusive};

use super::{
    to_u, Bin, BinModIterHelper, Binning, GenericHist, HasUnsignedVersion, HistErrors, Histogram,
    HistogramCombine,
};
...
use sampling::histogram::" [<Binning $t:upper>] ";\n\
                ...
                ```"]
                #[inline]
                pub fn native_bin_iter(&self) -> impl Iterator<Item=RangeInclusive<$t>>
                {
...
other_binning!(u8, i8, u16, i16, u32, i32, u64, i64, u128, i128, usize, isize);
...
mod tests {
    ...
    fn unit_test_distance() {
        // # bin width 1
        let binning = BinningI8::new_inclusive(-50, 50, 1).unwrap();
        ...
        // # bin width 2
        let binning = BinningI8::new_inclusive(-50, 49, 2).unwrap();
        let mut dist = binning.distance(i8::MIN);
        for i in i8::MIN + 1..-50 {
            let new_dist = binning.distance(i);
            assert!(dist > new_dist);
    ...
    #[test]
    fn test_combining() {
        let binning_1 = BinningI16::new_inclusive(-10, 9, 2).unwrap();
        let hist_1 = GenericHist::new(binning_1);

        let binning_2 = BinningI16::new_inclusive(10, 11, 2).unwrap();
        let hist_2 = GenericHist::new(binning_2);

        let encapsulating = GenericHist::encapsulating_hist(&[&hist_1, &hist_2]).unwrap();

        assert_eq!(encapsulating.binning().first_border(), -10);
        assert_eq!(encapsulating.binning().last_border(), 11);

        let binning_3 = BinningI16::new_inclusive(12, 15, 2).unwrap();
        let hist_3 = GenericHist::new(binning_3);

        let encapsulating = GenericHist::encapsulating_hist(&[&hist_3, &hist_1]).unwrap();

        assert_eq!(encapsulating.binning().first_border(), -10);
        assert_eq!(encapsulating.binning().last_border(), 15);

        let misaligned_binning = BinningI16::new_inclusive(-11, -10, 2).unwrap();
        let misaligned_hist = GenericHist::new(misaligned_binning);

        match GenericHist::encapsulating_hist(&[&hist_3, &hist_1, &hist_2, &misaligned_hist]) {
            Ok(_) => panic!("Bug in code! This has to give the error variant!"),
            Err(err) => {
                assert_eq!(HistErrors::ModuloError, err);
            }
        };

        let binning_4 = BinningI16::new_inclusive(-12, -11, 2).unwrap();
        ...
        match GenericHist::encapsulating_hist(&[
            ...
            Ok(_) => panic!("Bug in code! This has to give the error variant!"),
            Err(err) => {
                assert_eq!(HistErrors::Alignment, err);
            }
        };
    }
    ...
    fn index_test() {
        ...
        let iter = uniform.sample_iter(rng).take(10000);
        for val in iter {
            eprintln!("{val}");
